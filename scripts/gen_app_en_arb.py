        ...
        messages=[
            {
                "role": "system",
                "content": """
            Flutterで作られたピル管理アプリ・Pilllの翻訳作業を手伝ってください
            渡された日本語に対して適切な .arb のkeyを生成してください。keyは英訳したものをUpper Camel Case したものにして欲しいです
             """,
            },
            {
                "role": "user",
                "content": f"Translate the following text to .arb file key: '{text}'",
            },
        ],
        functions=[
            {
...
def translate_english_value(text):
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"Translate the following text into English for .arb value: '{text}'",
            },
        ],
        functions=[
            {
