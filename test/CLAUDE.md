...
**注意**: PillSheetType に限っては `for value in PillSheetType.values` のようなテストは避け、必要なものは個別で静的に group/test を作成しましょう。種類ごとに錠数や偽薬期間の有無が異なり、動的なループでは意図が不明確になるため。多くの場合は服用する期間が変わる程度の違いしかないので重複するテストが無闇に増えるのも気になります
