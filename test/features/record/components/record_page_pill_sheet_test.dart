...
void main() {
  ...
  group("#PillNumber", () {
    group("pillSheetAppearanceMode is number", () {
      ...
      testWidgets("it is isPremium or isTrial", (WidgetTester tester) async {
        ...
        for (int i = 0; i < 28; i++) {
          ...
          final widget = PillNumber(
            ...
            pageIndex: 0,
            setting: setting,
            user: FakeUser(fakeIsPremium: true, fakeIsTrial: true),
          );
          await tester.pumpWidget(MaterialApp(home: widget));

          if (pillNumberInPillSheet < pillNumberForFromMenstruation) {
            expect(
              find.byWidgetPredicate((widget) => widget is PlainPillNumber),
              findsWidgets,
              reason: "pillNumberInPillSheet: $pillNumberInPillSheet",
            );
          } else if (pillNumberInPillSheet <
              pillNumberForFromMenstruation + durationMenstruation) {
            expect(
              find.byWidgetPredicate(
                (widget) => widget is MenstruationPillNumber,
              ),
              findsWidgets,
              reason: "pillNumberInPillSheet: $pillNumberInPillSheet",
            );
      ...
      testWidgets("it is not isPremium and isTrial", (
        WidgetTester tester,
      ) async {
        SupportedDeviceType.iPhone5SE2nd.binding(tester.view);
        ...
        for (int i = 0; i < 28; i++) {
          final pillNumberInPillSheet = i + 1;
          final widget = PillNumber(
            user: FakeUser(fakeIsPremium: false, fakeIsTrial: false),
            pillSheetGroup: pillSheetGroup,
            pillSheet: pillSheet,
            pillNumberInPillSheet: pillNumberInPillSheet,
            pageIndex: 0,
  ...
  group("#RecordPagePillSheet.displayPillTakeDate", () {
    ...
    group("it is have rest duration", () {
      group("it is not ended rest duration", () {
        test(
          "simualte begin rest duration. pillSheet.lastTakenDate is yesterday and restDuration.beginDate is mockToday.",
          () {
            final originalTodayRepository = todayRepository;
            final mockTodayRepository = MockTodayService();
            todayRepository = mockTodayRepository;
            ...
            expect(
              pillSheet.displayPillTakeDate(1),
              DateTime.parse("2020-09-01"),
            );
            expect(
              pillSheet.displayPillTakeDate(2),
              DateTime.parse("2020-09-02"),
            );
            expect(
              pillSheet.displayPillTakeDate(3),
              DateTime.parse("2020-09-03"),
            );

            expect(
              pillSheet.displayPillTakeDate(10),
              DateTime.parse("2020-09-10"),
            );
            expect(
              pillSheet.displayPillTakeDate(11),
              DateTime.parse("2020-09-11"),
            );
            expect(
              pillSheet.displayPillTakeDate(12),
              DateTime.parse("2020-09-12"),
            );

            expect(
              pillSheet.displayPillTakeDate(26),
              DateTime.parse("2020-09-26"),
            );
            expect(
              pillSheet.displayPillTakeDate(27),
              DateTime.parse("2020-09-27"),
            );
            expect(
              pillSheet.displayPillTakeDate(28),
      ...
      group("it is ended rest duration", () {
        ...
        test(
          ...
          () {
            ...
            expect(
              pillSheet.displayPillTakeDate(13),
              DateTime.parse("2020-09-14"),
            );
            expect(
              pillSheet.displayPillTakeDate(14),
        ...
        test("Real bug case ", () {
          ...
          expect(
            pillSheet.displayPillTakeDate(24),
            DateTime.parse("2022-01-09"),
          );

          // Bug: Got 2022-01-18
          expect(
            pillSheet.displayPillTakeDate(25),
    ...
    test(
      "Bugfix 2023-02-11 dev:/users/Ka9rvL7WdFfmV7ZJe24XD9QfBAF2/pill_sheet_groups/pPJ3ncti5k3HfvbxU5Xe",
      () async {
        final originalTodayRepository = todayRepository;
        final mockTodayRepository = MockTodayService();
        todayRepository = mockTodayRepository;
        when(
          mockTodayRepository.now(),
        ).thenReturn(DateTime.parse("2023-01-14"));
        when(
          mockTodayRepository.now(),
        ).thenReturn(DateTime.parse("2023-01-14"));
        addTearDown(() {
