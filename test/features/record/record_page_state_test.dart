...
void main() {
  ...
  group("#markFor", () {
    test("it is alredy taken all", () async {
      ...
      expect(pillSheetGroup.pillSheets.first.todayPillNumber, pillSheetGroup.pillSheets.first.lastTakenOrZeroPillNumber);
  ...
  group("#remainingPillTakenCountFor", () {
    group("v1の場合", () {
      test("実薬期間でもnullが返される（1錠飲みユーザーは数字表示なし）", () async {
        ...
        await waitForResetStoreState();
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 1, pillSheet: pillSheetEntity), isNull);
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 22, pillSheet: pillSheetEntity), isNull);
      });
    });
    ...
    group("v2 実薬期間", () {
      test("未服用の場合、takenCount（2）が返される", () async {
        ...
        await waitForResetStoreState();
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 1, pillSheet: pillSheetEntity), 2);
      });
      ...
      test("1回服用済みの場合、1が返される", () async {
        ...
        await waitForResetStoreState();
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 1, pillSheet: updatedPillSheet), 1);
      });
      ...
      test("2回服用済みの場合、nullが返される（完了）", () async {
        ...
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 1, pillSheet: pillSheetEntity), isNull);
        // 2日目は未服用なので2
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 2, pillSheet: pillSheetEntity), 2);
      });
    });
    ...
    group("v2 休薬期間（pillsheet_21の22-28日目）", () {
      test("未服用の場合、takenCount（2）が返される", () async {
        ...
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 22, pillSheet: pillSheetEntity), 2);
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 28, pillSheet: pillSheetEntity), 2);
      });
    });
    ...
    group("v2 偽薬期間（pillsheet_28_4の25-28日目）", () {
      test("未服用の場合、takenCount（2）が返される", () async {
        ...
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 25, pillSheet: pillSheetEntity), 2);
        expect(remainingPillTakenCountFor(pillNumberInPillSheet: 28, pillSheet: pillSheetEntity), 2);
      });
    });
  });
