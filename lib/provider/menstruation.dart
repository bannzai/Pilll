...
final allMenstruationProvider = StreamProvider<List<Menstruation>>(
  (ref) => ref
      ...
      .map(
        (value) => value.where((element) => element.deletedAt == null).toList(),
      ),
);
...
final beginMenstruationProvider = Provider(
  (ref) => BeginMenstruation(ref.watch(databaseProvider)),
);
...
class BeginMenstruation {
  ...
  Future<Menstruation> call(DateTime begin, DateTime end) async {
    ...
    if (await _canHealthkitDataSave()) {
      final healthKitSampleDataUUID = await addMenstruationFlowHealthKitData(
        menstruation,
      );
...
final setMenstruationProvider = Provider(
  (ref) => SetMenstruation(ref.watch(databaseProvider)),
);
...
class SetMenstruation {
  ...
  Future<Menstruation> call(Menstruation menstruation) async {
    ...
    final doc = databaseConnection.menstruationReference(
      mutableMenstruation.id,
    );
...
final deleteMenstruationProvider = Provider(
  (ref) => DeleteMenstruation(ref.watch(databaseProvider)),
);
...
class DeleteMenstruation {
  ...
  Future<void> call(Menstruation menstruation) async {
    await databaseConnection.menstruationReference(menstruation.id).update({
      MenstruationFirestoreKey.deletedAt: now(),
    });
