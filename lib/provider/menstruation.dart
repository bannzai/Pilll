...
final latestMenstruationProvider = Provider((ref) => ref
    .watch(allMenstruationProvider)
    .whenData((menstruations) => menstruations.firstOrNull));

final beginMenstruationProvider =
    Provider((ref) => BeginMenstruation(ref.watch(databaseProvider)));
...
class BeginMenstruation {
  ...
  Future<Menstruation> call(DateTime begin, DateTime end) async {
    var menstruation =
        Menstruation(beginDate: begin, endDate: end, createdAt: now());
...
final setMenstruationProvider =
    Provider((ref) => SetMenstruation(ref.watch(databaseProvider)));
...
class SetMenstruation {
  ...
  Future<Menstruation> call(Menstruation menstruation) async {
    ...
    if (mutableMenstruation.id == null) {
      if (await _canHealthkitDataSave()) {
        final healthKitSampleDataUUID =
            await addMenstruationFlowHealthKitData(mutableMenstruation);
    ...
    final doc =
        databaseConnection.menstruationReference(mutableMenstruation.id);
    await doc.set(mutableMenstruation, SetOptions(merge: true));
    return mutableMenstruation.copyWith(id: doc.id);
  }
}

final deleteMenstruationProvider =
    Provider((ref) => DeleteMenstruation(ref.watch(databaseProvider)));
...
class DeleteMenstruation {
  ...
  Future<void> call(Menstruation menstruation) async {
    await databaseConnection
        .menstruationReference(menstruation.id)
        .update({MenstruationFirestoreKey.deletedAt: now()});
