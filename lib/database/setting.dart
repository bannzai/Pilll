...
final settingDatastoreProvider = Provider((ref) => SettingDatastore(ref.watch(databaseProvider)));

final settingStreamProvider = StreamProvider((ref) => ref.watch(settingDatastoreProvider).stream());
...
class SettingDatastore {
  ...
  Future<Setting> update(Setting setting) {
    return _database.userRawReference().update({UserFirestoreFieldKeys.settings: setting.toJson()}).then((_) => setting);
  }
