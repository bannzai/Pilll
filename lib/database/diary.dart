...
final diariesStreamAround90Days = StreamProvider.family((ref, DateTime base) => ref.watch(diaryDatastoreProvider).streamForAround90Days(base: base));
...
class DiaryDatastore {
  ...
  Future<List<Diary>> fetchListForMonth(DateTime dateTimeOfMonth) {
    return _database
        .diariesReference()
        .where(DiaryFirestoreKey.date,
            isLessThanOrEqualTo: DateTime(dateTimeOfMonth.year, dateTimeOfMonth.month + 1, 0),
            isGreaterThanOrEqualTo: DateTime(dateTimeOfMonth.year, dateTimeOfMonth.month, 1))
        .orderBy(DiaryFirestoreKey.date)
        .get()
        .then((event) => event.docs.map((e) => e.data()).toList());
  }
