  ...
  Future<PillSheetGroup> call({
    ...
    for (final pillSheet in pillSheetGroup.pillSheets) {
      if (pillSheet.id == activePillSheet.id) {
        ...
        updatedPillSheets.add(pillSheet.copyWith(beginDate: beforeUpdatedPillSheet.estimatedEndTakenDate.add(const Duration(days: 1))));
      } else {
...
class ChangeRestDuration {
  ...
  Future<void> call({required RestDuration fromRestDuration, required RestDuration toRestDuration, required PillSheetGroup pillSheetGroup}) async {
    ...
    for (final pillSheet in updatedRestDurationPillSheets) {
      ...
      updatedBeginingDatePillSheets.add(pillSheet.copyWith(beginDate: beforePillSheet.estimatedEndTakenDate.date().addDays(1)));
    }
