...
class InitialSettingState with _$InitialSettingState {
  ...
  Future<Setting> buildSetting() async {
    ...
    final pillNumberForFromMenstruation =
        max(0, maxPillCount - menstruationDuration);
  ...
  static DateTime _beginingDate({
    ...
    if (pageIndex <= todayPillNumber.pageIndex) {
      ...
      return today().subtract(Duration(
          days:
              passedTotalCount + (todayPillNumber.pillNumberInPillSheet - 1)));
    } else {
      ...
      return beforePillSheetBeginingDate
          .addDays(beforePillSheetType.totalCount);
    }
  }
