...
App Store 用スクリーンショットを、シミュレータ・実機ビルドなしで生成するための Widget 群。

`flutter test` の widget test（`test/appstore_screenshot/generate_screenshots_test.dart`）が
ここの Widget を組み立てて 1290×2796 の PNG として書き出す。撮影・出力の仕組みや
実行スクリプトは `scripts/generate_screenshots/README.md` を参照。
...
| `screenshot_text.dart` | 言語×ページ → キャッチコピー（未定義言語は en フォールバック） |
...
- **UI 言語とコピーの両方をロケール切替**: 撮影ハーネスが `MaterialApp.locale` と
  ...
  スクショ固有の短い文言（バッジの利用者数・曜日ラベル等）は Mock に渡す `lang` で切り替える。
- **タイトルの折り返し**: タイトルは明示改行（`\n`）だけで改行し、幅を超える行は
...
- **言語追加**: `screenshot_text.dart` の `_copies` に 1 言語分の Map を追記する。
  Mock 固有文言が必要なら各 Mock 内の言語マップにも追記する。
