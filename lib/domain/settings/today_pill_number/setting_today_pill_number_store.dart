...
final settingTodayPillNumberStoreProvider =
    StateNotifierProvider.autoDispose.family<SettingTodayPillNumberStateStore, SettingTodayPillNumberState, SettingTodayPillNumberStoreParameter>(
...
class SettingTodayPillNumberStateStore extends StateNotifier<SettingTodayPillNumberState> {
  ...
  Future<void> modifiyTodayPillNumber({
    ...
    pillSheetGroup.pillSheets.asMap().keys.forEach((index) {
      ...
      if (state.selectedPillSheetPageIndex == index) {
        lastTakenDate = beginDate.add(Duration(days: state.selectedPillMarkNumberIntoPillSheet - 2));
